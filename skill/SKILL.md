...
MCP 客户端使用 `npm run mcp`，通过 JSON-RPC stdio 调用 `publishing_import`、`publishing_guidance`、`publishing_state`、`publishing_draft_status`、`publishing_draft_submit`、`publishing_apply_text`、`publishing_humanize`、`publishing_apply_intent`、`publishing_export` 和 `publishing_publish`。`publishing_draft_submit` 必须传 `confirm: true` 才允许产生远程提交副作用。状态默认写入 `.local-data/document.json`，也可通过 `WECHAT_LAYOUT_DATA` 指定路径。
